...
psi<-as.numeric(args[15])
viral_decay<-as.numeric(args[16])
plaques<-as.numeric(args[17])
...
write("model,k,log beta,log viral prod,log viral diff,log cyto diff,log cyto uptake,log inf ic50,log beta ic50,log prod ic50,log cyt_trm_ic50,cyt_inf_death,hsv_fract,peak VL RSS,peak VL AIC,peak time RSS,peak time AIC,duration RSS,duration AIC,cat2 RSS,cat2 AIC,cat3 RSS,cat3 AIC,psi,viral decay,plaques", file="scores.csv",append=FALSE)

all_data <- read.csv("../../trial_data/All_episodes_summstats.csv", comment.char="#")
#epID,ep_type,ep_dur,n_swabs,mean_vl,time_to_first_peak,first_peak,max_vl,time_to_max,first_pos,last_pos,any_symp,any_lesion,n_sym,n_lesion
x=colnames(all_data)
base_peak<-sort(as.numeric(all_data[,7]),decreasing=TRUE)
base_peak[base_peak>7]<-7
base_peak_time<-sort(as.numeric(all_data[,6])/24,decreasing=TRUE)
base_peak_time[base_peak_time>5]<-5
base_days<-sort(as.numeric(all_data[,3])/24,decreasing=TRUE)
base_days[base_days>5]<-5
base_first<-sort(as.numeric(all_data[,11]),decreasing=TRUE)
...
mean_peak = mean(base_peak)
mean_peak_time = mean(base_peak_time)
mean_days = mean(base_days)
print(paste("Base data mean log peak is ",mean_peak))
print(paste("Base data mean peak time is ",mean_peak_time))
print(paste("Base data mean duration is ",mean_days))
...
peak_score <- 0
n<-min(nrow(all_data),nrow(run_data))
for (i in 1:n) {
    peak_score = peak_score + ((peak_vl[i] - base_peak[i])/mean_peak)**2
}
...
peak_time_score <- 0
for (i in 1:n) {
    peak_time_score = peak_time_score + ((peak_time[i] - base_peak_time[i])/mean_peak_time)**2
}
...
days_score <- 0
for (i in 1:n) {
    days_score = days_score + ((days[i] - base_days[i])/mean_days)**2
}
...
write(paste(model,k,beta,viral_prod,viral_diff,cyt_diff,
	...
	cat2_rss_score,cat2_AIC_score, cat3_rss_score,cat3_AIC_score,psi,viral_decay,plaques,
